...
Parser.prototype = {
    ...
    button: function () {
        var buttonInfo = this.advance();
        return {
            type: 'button',
            value: buttonInfo.value,
            attrs: buttonInfo.attrs
        }
    },
    ...
    include: function () {
        ...
        if (src) {
            ...
            var dirName = path.dirname(this.options.path);
            var joinedPath = path.join(dirName, options.path);
            var readFile = fs.readFileSync(joinedPath, 'utf8');

            parser = new Parser(readFile, this.options);
